...
class Gui(Tk):
    ...
    def row(self, weights=[1], **options):
        """create a frame that lays out widgets in a single row."""
        return self.gr(65536, weights, [1], **options)

    def col(self, weights=[1], **options):
...
def widget_demo():
    ...
    # a label
    la1 = g.la(side=TOP, text='This is a label.')

    # an entry
    en = g.en(side=TOP, fill=NONE)
    en.insert(END, 'This is an entry widget.')

    # another label
    la2 = g.la(side=TOP, text='')
    ...
    # menubutton
    mb = g.mb(side=BOTTOM, text='Choose a color')
    ...
    # button
    bu = g.bu(side=BOTTOM, text='Apply color', command=apply_color)
    ...
    for size in [10, 12, 14, 15, 17, 20]:
        rb = g.rb(side=TOP, text=str(size), variable=fontsize, value=size,
                  command=set_font)

    # make the check buttons
    b1 = g.cb(side=TOP, text='Bold', command=set_font, variable=StringVar(),
              onvalue=BOLD, offvalue=NORMAL)
    b1.deselect()
    
    b2 = g.cb(side=TOP, text='Italic', command=set_font, variable=StringVar(),
    ...
    # text widget
    te = g.te(side=TOP, fill=X, height=5, width=40)
